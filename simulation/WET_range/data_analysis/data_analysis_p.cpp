...
struct particle {
    ...
    void CalculateDoses(const int channel){ //water equivalence
        offset = 5*channel+3*channel;
        double prev_proj = 5*channel+3*channel;
        double proj = 0; 
        double prev_diff_h2o = 0;
        double prev_diff_pwo = 0;
        double dose_change;

        for(int i = 0; i<fprojZ.at(channel).size(); i++){
            proj = std::round(fprojZ.at(channel).at(i)*1/resolution);
            auto dose_pair = fDose.at(channel).find(proj);
            if(dose_pair != fDose.at(channel).end()){ //find existing pair
                dose_pair->second->AddDose(dE.at(channel).at(i));                                
            }
            else{                                                                
                std::shared_ptr<dose> newDose = std::make_shared<dose>(); //create new pair
                newDose->AddDose(dE.at(channel).at(i));
                fDose.at(channel)[proj] = newDose;
            }
            
            double conv_factor = dEdX.at(channel).at(i)/StoppingPowerMap[std::round(feKin.at(channel).at(i)*1/resolution)];///bethe_bloch(fbeta.at(channel).at(i), I_h2o, 7.42, 18.02, 0.997, 1);
            conv_factor = 1;
            WEdepth += conv_factor*(fprojZ.at(channel).at(i)-prev_proj);                                    

            //CalculateWaterEquivalentProjection(channel, proj*resolution, prev_proj*resolution, dEdX.at(channel).at(i), fbeta.at(channel).at(i));
            //double WEdepth = CalculateWaterEquivalentDepth(channel, std::ceil(fprojZ.at(channel).at(i)*1/resolution)*resolution-offset);
            auto WEdose_pair = fWEDose.at(channel).find(std::round(WEdepth*1/(10*resolution)));
            //cout << "std::abs(WEdepth-WEdepth_prev) " << std::abs(WEdepth-WEdepth_prev) << endl;
            double diff_h2o = std::abs(WEdepth-WEdepth_prev);
            double diff_pwo = std::abs(fprojZ.at(channel).at(i)-prev_proj);
            
            // if(diff_h2o == 0 ) dose_change = 0;
            // else if(diff_pwo == 0 ) dose_change = 0;
            // else dose_change = dE.at(channel).at(i);//*diff_pwo/diff_h2o;
            dose_change = dE.at(channel).at(i);
            
            if(WEdose_pair != fWEDose.at(channel).end()){ //find existing pair
                WEdose_pair->second->AddDose(dose_change);
            }
            else{                                                                
                std::shared_ptr<dose> newWEDose = std::make_shared<dose>(); //create new pair
                newWEDose->AddDose(dose_change);
                fWEDose.at(channel)[std::round(WEdepth*1/(10*resolution))] = newWEDose;                
            }
            WEdepth_prev = WEdepth;
            prev_proj = fprojZ.at(channel).at(i);
            prev_diff_h2o = diff_h2o;
            prev_diff_pwo = diff_pwo;
        }
        return;
    }
    ...
    double CalculateWaterEquivalentProjection(int channel, double m_proj, double m_prev_proj, double m_dEdX, double m_beta){
        if(channel < fnbofdetectorsPbWO4){
            return m_dEdX/bethe_bloch(m_beta,I_h2o, 7.42, 18.02, 0.997, 1)*(m_proj-m_prev_proj);
        }
        else{
            return WaterEquivalentLength(m_beta, rho_pen);
        }
    }
    ...
    void ProcessParticle(){
        ...
        for(int i=0; i<edep.size(); i++){
            ...
            CalculateEDep_idx(i);
            CalculateDoses(i);
            double air_space = WaterEquivalentLength(3, rho_air);
            WEdepth += air_space;
            WEdepth_prev += air_space;
        }
        return;
    }
...
void data_analysis_p(double dLayer = 0){
    ...
    if(nLayer == 0){
        ...
        sprintf(filename, "raw_data_%i.root", nLayer);
    }
    ...
    sprintf(filename, "range_%i.root", nLayer);
    ...
    for(particle p : particles){
        ...
        p.ProcessParticle();
        p.GetTotalEDep();
        //cout << "inital Energy " << p.initialEnergy << " range " << p.range << endl;
        tree_range = p.range;
        h_range->Fill(tree_range);
        outputtree->Fill();
        // DetectorTheory.SetDoses(p.fDose, p.fWEDose);
        // for(const auto& [depth, dose] : p.fWEDose.at(0)){
        //     h2_curve->Fill(depth, dose->GetDose()); 
        // }
    }
    ...
    simple_plot(h_range, "Range / mm", "Counts");
}
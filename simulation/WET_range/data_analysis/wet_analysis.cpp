...
double alpha_w = 2.354e-3;
double p_w = 1.757;

//-------------energy range relationship of pbwo4 --------
double alpha_pbwo4 = 6.7642e-4;
double p_pbwo4 = 1.7052;


std::unordered_map<int, double> StoppingPowerMap;

double CalcEnergyPow(double energy, double p){
    return std::pow(energy, 2-p);
}

double CalculateWET(double t_m, double E_i, double E_f, double alpha_m, double p_m){
    return t_m *((alpha_w*p_w*(2-p_w))/(alpha_m*p_m*(2-p_m)))*((CalcEnergyPow(E_i, p_m)-CalcEnergyPow(E_f, p_m))/(CalcEnergyPow(E_i, p_w)-CalcEnergyPow(E_f, p_w)));
}
...
void wet_analysis(){
    ...
    int energy = 0; //energy storage for hist fill
    ...
    double j = 0;
    for(int i = 0; i< DetectorTheory.fTotalDose.size(); i++){
    for(const auto& [depth, dose] : DetectorTheory.fTotalDose.at(i)){
        j++;
        g_detectortheory->SetPoint(j, depth, dose->GetDose());
    }
    }
    // simple_plot(h_range, "Range / mm", "Counts");
    plot_tgraph(g_detectortheory, "Water Equivalent Depth / mm", "Total Dose / MeV");
}
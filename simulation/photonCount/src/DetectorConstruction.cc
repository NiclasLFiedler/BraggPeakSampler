...
void DetectorConstruction::DefineMaterials()
{
  ...
  pbwo4MPT->AddConstProperty("SCINTILLATIONYIELD", 100./MeV); // Adjust yield
  pbwo4MPT->AddConstProperty("RESOLUTIONSCALE", 1.0);
  pbwo4MPT->AddConstProperty("SCINTILLATIONTIMECONSTANT1", 10.*ns);
  pbwo4MPT->AddConstProperty("SCINTILLATIONTIMECONSTANT2", 50.*ns);
  ...
  // if(detectorType == "pbwo4"){
    ...
    detMaterial->GetIonisation()->SetBirksConstant(0.01268*mm/MeV);
    // detMaterial->GetIonisation()->SetBirksConstant(0.0000001*mm/MeV);
    // detMaterial->GetIonisation()->SetBirksConstant(0.154*mm/MeV);
...
G4VPhysicalVolume* DetectorConstruction::DefineVolumes()
{
  ...
  G4double SiPMSizeY = 3.7*mm;
  G4double SiPMSizeZ = 3.7*mm;
  if(SiPMSizeZ>detSizeZ) SiPMSizeZ = detSizeZ;
  G4double SiPMSizeX = ThicknessTeflon/2;
  G4Box* solidSiPM = new G4Box("solidSiPM", SiPMSizeX/2, SiPMSizeY/2, SiPMSizeZ/2);

...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  if (particle == G4ChargedGeantino::ChargedGeantino()) {
    ...
    G4double E_max = 220.*MeV;
    G4double rand = G4UniformRand();
    fParticleGun->SetParticleEnergy(E_min + rand * (E_max - E_min));
    ...
    beamPosIsocentre = G4ThreeVector(x0+PosNozzle.x(), y0+PosNozzle.y(), Pos_Isocentre.z()-5*cm);
    fParticleGun->SetParticleMomentumDirection(beamPosIsocentre-beamPosNozzle);
    
    beamPos = beamPosIsocentre;
  }
